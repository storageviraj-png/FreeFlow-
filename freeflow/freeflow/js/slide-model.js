...
// A text/image element positioned on the slide canvas. x/y/w/h are all
// PERCENTAGES of the 1920x1080 virtual canvas (0-100), so the exact same
// numbers render identically in the controller's preview and in output.html
// regardless of actual pixel size.
export function newElement(overrides = {}) {
  return Object.assign({
    id: newId('el'),
    kind: 'text',              // 'text' | 'image'
    content: '',
    src: '',                   // image src, only used when kind === 'image'
    ...
    imageFit: 'contain',        // cover | contain | fill (kind === 'image' only)
    zIndex: 1
  }, overrides);
}
